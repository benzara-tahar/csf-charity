...
import { AdminsEditComponent } from './edit/edit.component';
...
export class AdminsListComponent implements OnInit {
  ...
  showNewModal() {
    this.modal.createStatic(AdminsEditComponent, {}).subscribe(() => this.st.reload());
  ...
  showEditModal(data: User) {
    ...
    this.modal.createStatic(AdminsEditComponent, params).subscribe(() => this.st.reload());
    this.modalService.openModals[0].afterClose.subscribe((response) => {
      if (response) {
        this.getAdmins();
      }
